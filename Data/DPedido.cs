    ...
    public class DPedido
    {
        public List<Pedido> GetPedidos(Pedido pedido)
        {
            ...
            try
            {
                ...
                using (SqlDataReader reader = SQLHelper.ExecuteReader(SQLHelper.Connection, "USP_ListarPedidosEntreFechas", CommandType.StoredProcedure, parameters))
                {
                    while (reader.Read())
                    {
